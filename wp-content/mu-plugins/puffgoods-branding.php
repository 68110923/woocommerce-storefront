<?php
/**
 * Plugin Name: Puffgoods Frontend Design
 * Description: Puffgoods 前台设计(公共/桌面/移动 css+js 分离)与购物车抽屉、页脚。
 * Version: 3.0
 */
...
function puffgoods_enqueue_assets() {
    if ( is_admin() ) return;
    $base = content_url( 'mu-plugins/puffgoods-assets' );
...
add_action( 'wp_enqueue_scripts', 'puffgoods_enqueue_assets' );


/* ---------------- 产品轮播短代码 ---------------- */
function puffgoods_products_carousel( $atts ) {
...
add_shortcode( 'pg_products_carousel', 'puffgoods_products_carousel' );

/* ---------------- 页脚:购物车抽屉 + 顶部页头购物车按钮 ---------------- */
function puffgoods_footer_drawer() {
    if ( is_admin() ) return;
    ?>
...
add_action( 'wp_footer', 'puffgoods_footer_drawer' );

/* 顶部购物车数量角标随加购实时更新(WooCommerce fragments) */
function puffgoods_cart_fragment( $fragments ) {
    $count = ( function_exists( 'WC' ) && WC()->cart ) ? (int) WC()->cart->get_cart_contents_count() : 0;
    $fragments['span.pg-cart-count'] = '<span class="pg-cart-count">' . esc_html( $count ) . '</span>';
    return $fragments;
}
add_filter( 'woocommerce_add_to_cart_fragments', 'puffgoods_cart_fragment' );

/* ---------------- 自定义页脚 ---------------- */
function puffgoods_footer() {
    if ( is_admin() ) return;
    ?>
    <footer class="pg-footer">
      <div class="pg-container">
        ...
        <div class="pg-bottom">
          <span>&copy; 2026 Puffgoods. All rights reserved. Melbourne, Victoria, Australia. <a href="mailto:puffgoods.co@gmail.com">puffgoods.co@gmail.com</a>.</span>
...
add_action( 'wp_footer', 'puffgoods_footer', 20 );
